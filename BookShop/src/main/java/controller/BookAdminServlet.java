...
public class BookAdminServlet extends HttpServlet {
	...
	private String saveImage(Part filePart, String folder) throws IOException {
		try (InputStream is = filePart.getInputStream(); ByteArrayOutputStream os = new ByteArrayOutputStream()) {
			...
			Map uploadResult = cloudinary.uploader().upload(os.toByteArray(), ObjectUtils.asMap("folder", folder));
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if ("add".equals(action)) {
			...
			if (title == null || title.trim().isEmpty()) {
				json(response, false, "empty_title");
				return;
			}
			if (price < 0 || stock < 0) {
			...
			if (originPrice < price) {
				json(response, false, "invalid_origin_price");
				return;
			}
			int currentYear = Calendar.getInstance().get(Calendar.YEAR);
			if (publishYear < 1000 || publishYear > currentYear) {
				json(response, false, "invalid_year");
				return;
			}
			...
			for (Part part : request.getParts()) {
				if ("subImages".equals(part.getName()) && part.getSize() > 0) {
					...
					if (subUrl != null)
						dao.addBookImage(bookId, subUrl);
				}
			}
