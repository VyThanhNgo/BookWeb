...
public class OrderDAO {
	...
	public boolean createFullOrder(Order order, Collection<CartItem> items) {
		...
		String insertDetailSql = "INSERT INTO order_details ("
				+ "order_id, book_id, book_title, quantity, unit_price, line_total" + ") VALUES (?, ?, ?, ?, ?, ?)";
	...
	public List<OrderItem> getOrderItems(int orderId) {
		...
		String sql = "SELECT od.*, b.image FROM order_details od " + "LEFT JOIN books b ON od.book_id = b.book_id "
				+ "WHERE od.order_id = ?";
		try (Connection conn = DBConnection.getConnection(); PreparedStatement ps = conn.prepareStatement(sql)) {
			ps.setInt(1, orderId);
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {
				OrderItem item = new OrderItem(rs.getInt("book_id"), rs.getString("book_title"), rs.getString("image"),
	...
	/**
	 * Cập nhật trạng thái đơn hàng và ghi nhận thanh toán vào bảng payments. Dùng
	 * sau khi nhận kết quả từ cổng thanh toán (VNPay / MoMo).
	 */
	public boolean updateOrderPayment(String orderCode, String orderStatus, String paymentStatus) {
		...
		try {
			...
			PreparedStatement ps1 = conn.prepareStatement("UPDATE orders SET status = ? WHERE order_code = ?");
			ps1.setString(1, orderStatus);
			ps1.setString(2, orderCode);
			ps1.executeUpdate();
			ps1.close();

			PreparedStatement ps2 = conn.prepareStatement("INSERT INTO payments (order_id, method, status, paid_at) "
					+ "SELECT order_id, payment_method, ?, ? FROM orders WHERE order_code = ?");
			ps2.setString(1, paymentStatus);
			ps2.setTimestamp(2,
					"SUCCESS".equals(paymentStatus) ? new java.sql.Timestamp(System.currentTimeMillis()) : null);
	...
	// sách COMPLETED trong 30 ngày và chưa review
	public List<Map<String, Object>> getUnreviewedBooks(int userId) {
		List<Map<String, Object>> list = new ArrayList<>();
		// trả về theo từng đơn hàng, mỗi đơn chứa list sách chưa review
		String sql = "SELECT o.order_id, o.order_code, o.created_at AS order_date, "
				...
				+ "od.id AS detail_id, od.book_id, od.book_title, b.image " + "FROM orders o "
	...
	public List<Map<String, Object>> getReviewedBooks(int userId) {
		...
		String sql = "SELECT r.review_id, r.rating, r.comment, r.created_at, "
				+ "b.book_id, b.image AS book_image, od.book_title " + "FROM reviews r "
				+ "JOIN order_details od ON od.book_id = r.book_id "
				+ "JOIN orders o ON o.order_id = od.order_id AND o.user_id = ? "
				+ "JOIN books b ON b.book_id = r.book_id " + "WHERE r.user_id = ? "
